"""FastAPI application for long-context-rag."""
...
app = FastAPI(title="long-context-rag", version="0.1.0", description="long-context-rag API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "long-context-rag", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "long-context-rag", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "long-context-rag", "version": "0.1.0", "ready": True,
